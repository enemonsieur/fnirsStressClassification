# -*- coding: utf-8 -*-
"""
Created on Wed Dec  7 06:40:08 2022
...
#%% This groups the channels detector between themso that we can compute a particular region of interest and not all the datas
...
results_subs=[]
for sub, value in path_dict.items(): #loop through the O6_sub, in the document with the values,
    raw_haemo_sc1, fnirs_data_filt=data_loading(os.path.join(baseFolder, sub, value[0])) #load the documents for the NIRx
    ...
    times, avg_data=supporting_func.get_time_avged_data(fnirs_data_filt) #it averages the data over all the sensors  
   ...
   # roi_index=list(combinations(list(np.arange(0, len(SD_list))), 2)) #use the sd list to group channels together

    #%% Loading epochs and compute features
    df_events=nirs_help.read_excel_sheets(os.path.join(baseFolder, sub, value[0], 'events.xlsx')) # load the ON and OFF times of the 06_SUB
    df_events['Predict']=df_events['Condition'].apply(lambda x: 1 if x=='Scenario_2' else 0) # label Scenario 1 and 2 = 0,1
    cols=['Onset', 'Offset'] #create an array Onset and Offset
    #epo_mean, epo_auc, epo_var, epo_max=[], [], [], []
    #pear_corr, spear_corr, cov_value=[], [], []
    X, Y = np.empty((df_events.shape[0],12971)),np.empty((df_events.shape[0],1))
    
    for index, row in df_events.iterrows(): #iterrates through the tasks that 06_SUB did
        ...
        epo_data= supporting_func.get_data(fnirs_data_filt.copy(), 'hbr', t)-base_avg # extract the 2D vector of fnirs data but only at a task 1
        epo_data_reshape= epo_data.flatten()
        X[index,:] = epo_data_reshape
        Y[index,:] = row['Predict']
            #redo the algorithm that didn't work
      #%%   
       ...
       # epo_max.append(np.amax(epo_data, axis=1)[np.newaxis, :])  # row: epochs, col: channels
        ...
        #epo_roi=nirs_help.get_row_avg_feat_idx(epo_data.T, grp_ch_index) #it finds the mean of the grouped channels in SD_list
       # pear_corr.append(compute_roi_corr(epo_roi, roi_index, 'Pearson')) #using those new channel, it finds the pearson corr between all of'em
